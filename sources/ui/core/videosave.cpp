...
void VideoSave::showError(int err) {
    ...
    qWarning() << "Error: " << m_error;
#else
    Q_UNUSED(err);
#endif
}

bool VideoSave::open(AVStream *inStream, QString const &filename) {
     ...
     * MJPEG压缩技术可以获取清晰度很高的视频图像，
        可以【动态调整帧率】适合保存摄像头视频、分辨率。但由于没有考虑到帧间变化，
        造成大量冗余信息被重复存储，因此单帧视频的占用空间较大；
     * 如果采用其它编码器，由于摄像头曝光时间长度不一定，所以录像时帧率一直在变，
        编码器指定固定帧率会导致视频一会快一会慢，效果很不好，
        适用于录制固定帧率的视频（当然其它编码器应该是有处理办法，不过我还不清楚）；
     */
    QString strName = avcodec_find_encoder(inStream->codecpar->codec_id)
                          ->name; // 获取编码器名称
    int ret = avformat_alloc_output_context2(
        &m_formatContext, nullptr, strName.toStdString().data(),
    ...
    ret = avio_open(&m_formatContext->pb, filename.toStdString().data(),
                    AVIO_FLAG_WRITE);
    if (ret < 0) {
        close();
        showError(ret);
    ...
    m_codecContext->pix_fmt = AVPixelFormat(
        inStream->codecpar
            ->format); // 像素格式，也可以使用codec->pix_fmts[0]或AV_PIX_FMT_YUVJ422P
                       // (【注意】摄像头解码的图像格式为yuvj422p，
                       // 如果这里不一样可能保存会出问题，或者后面进行格式转换)
#endif
    m_codecContext->time_base = {
        1,
        30}; // 设置时间基，30为分母，1为分子，表示以1/30秒时间间隔播放一帧图像
    ...
    ret = avcodec_open2(
        ...
        nullptr); // 使用h264时第一次打不开，第二次可以打卡，不知道什么原因
#endif
    if (ret < 0) {
        close();
        showError(ret);
...
void VideoSave::close() {
    ...
    QMutexLocker locker(
        &m_mutex); // 如果不加锁可能在点击关闭时，write函数正在写入数据，导致崩溃
    if (m_formatContext) {
        // 写入文件尾
        if (m_writeHeader) {
            m_writeHeader = false;
