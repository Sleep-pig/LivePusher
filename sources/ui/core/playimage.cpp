...
#if USE_WINDOW
PlayImage::PlayImage(QOpenGLWindow::UpdateBehavior updateBehavior,
                     QWindow *parent)
...
void PlayImage::repaint(AVFrame *frame) {
    ...
    if (!m_texY) // 初始化纹理
    {
        ...
        m_texU->setSize(frame->width / 2,
                        frame->height / 2); //   w,h      w/2,h      w/2,h/2
        m_texV->setSize(frame->width / 2,
                        frame->height / 2); //   w,h      w/2,h      w/2,h/2
...
void PlayImage::initializeGL() {
    ...
    glBufferData(GL_ARRAY_BUFFER,  // 为VBO缓冲绑定顶点数据
                 ...
                 vertices,         // 需要绑定的数组
                 GL_STATIC_DRAW); // 指定数据存储的预期使用模式,GL_STATIC_DRAW：
                                  // 数据几乎不会改变
    ...
    glVertexAttribPointer(
        ...
        GL_FALSE, // 指定在访问定点数据值时是否应规范化 ( GL_TRUE)
                  // 或直接转换为定点值 (
                  // GL_FALSE)，如果vertices里面单个数超过-1或者1可以选择GL_TRUE
        5 * sizeof(GLfloat), // 指定连续通用顶点属性之间的字节偏移量。
        nullptr); // 指定当前绑定到目标的缓冲区的数据存储中数组中第一个通用顶点属性的第一个组件的偏移量。初始值为0
                  // (一个数组从第几个字节开始读)
    // 启用通用顶点属性数组
    glEnableVertexAttribArray(
        posAttr); // 属性索引是从调用glGetAttribLocation接收的，或者传递给glBindAttribLocation。
...
void PlayImage::resizeGL(int w, int h) {
    ...
    if ((double(w) / h) < (double(m_size.width()) / m_size.height())) {
        ...
        m_zoomSize.setHeight(
            ((double(w) / m_size.width()) *
             m_size
                 .height())); // 这里不使用QRect，使用QRect第一次设置时有误差bug
    } else {
...
void PlayImage::paintGL() {
    ...
    glViewport(m_pos.x(), m_pos.y(), m_zoomSize.width(),
               m_zoomSize.height()); // 设置视图大小实现图片自适应
